...
class File {
	...
	public static function upload_file() {
		if ( isset( $_FILES['file'] ) ) {
			$file_input = $_FILES['file']; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			if ( sanitize_text_field( $file_input['error'] ) == UPLOAD_ERR_OK ) {
				...
				switch ( strtolower( sanitize_text_field( $file_input['type'] ) ) ) {
					//allowed file types
					case 'text/csv':
					case 'text/plain':
						break;
				...
				$file_name     = strtolower( sanitize_text_field( $file_input['name'] ) );
				...
				if ( move_uploaded_file( sanitize_text_field( $file_input['tmp_name'] ), $tmp_file_name ) ) {
