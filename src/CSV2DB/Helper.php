	...
	public static function check_environment() {
		...
		if ( ! self::php_version_gte( CSV2DB_SUPPORT_PHP ) ) {
			add_action( 'admin_notices', function () {
				...
				. esc_html_e( sprintf( '<strong>CSV To DB</strong> requires PHP version %s or later.',
					CSV2DB_SUPPORT_PHP ), 'csv2db' )
				. _e( '</p></div>' );
			} );
			$is_ok = false;
		}
		...
		if ( ! self::wp_version_gte( CSV2DB_SUPPORT_WP ) ) {
			add_action( 'admin_notices', function () {
				_e( '<div class="error"><p>' )
				. esc_html_e( sprintf( '<strong>CSV To DB</strong> requires WordPress version %s or later.',
					CSV2DB_SUPPORT_WP ), 'csv2db' )
				. _e( '</p></div>' );
			} );
			$is_ok = false;
		}
